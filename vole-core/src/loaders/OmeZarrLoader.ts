...
const DEFAULT_FETCH_OPTIONS = {
  maxPrefetchDistance: [12, 5, 5, 5] as [number, number, number, number],
...
class OMEZarrLoader extends ThreadableVolumeLoader {
  ...
  static async createLoader(
    ...
    /**
     * Optional pre-built zarrita stores, one per source (aligned with `urls`).
     * When a store is provided for index `i`, it is used instead of fetching
     * `urls[i]` over HTTP; the corresponding `urls[i]` then serves only as a
     * logical label for caching and error messages. Used for local-file sources
     * such as {@link ZipStore}.
     */
    stores?: (AsyncReadable<unknown> | undefined)[]
    ...
    const sourceProms = urlsArr.map(async (url, i) => {
      const store = stores?.[i] ?? new RelaxedFetchStore(url);
